...
'''
    gt_dir = 验证集标签 ground_truth所在路径
    pred_dir = 验证集模型预测输出结果所在路径
'''
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    for ind in range(len(gt_imgs)):
        ...
        # ------------------------------------------------#
        pred = np.array(Image.open(pred_imgs[ind])).astype('uint8')
        # ------------------------------------------------#
        #   读取一张对应的标签，转化成numpy数组
        # ------------------------------------------------#
        label = np.array(Image.open(gt_imgs[ind])).astype('uint8')
    ...
    return np.array(hist, dtype=np.int), IoUs, PA_Recall, Precision
